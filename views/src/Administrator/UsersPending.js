 ...
 */
 const HandleLetter =(data)=>{
    ...
    axios.get(`http://localhost:9020/${downloadID}`,{responseType: 'arraybuffer'})
        .then((response) => { 
 ...
 * @returns 
 */
export default function PenddedCustomes() {
    ...
    */
    const Columns = [
        ...
        { title: "Login Try", field: "loginTryCount"},
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const { data: response } = await axios.get(`http://localhost:9020/api/users/v1/pended-users/`);
                setData(response);
            } catch (error) {
        ...
        <HOMakerNavbar />
            <MuiThemeProvider theme={theme}>
                <div className={classes.appMain}>
                    <Header
                        title="Admins Template for pended Customers"
                        subTitle="Admins Controller to handle all pended users due to issues like  trying more thatn two login fails! "
